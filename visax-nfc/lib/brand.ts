/**
 * ─────────────────────────────────────────────────────────────
 *  LOGOTIPO VISAX NFC — geometría vectorial única (isotipo de VISAX AI)
...
/** Wordmark "VISAX NFC" (coordenadas en caja 992 × 105, altura de mayúscula 105). */
const WORD = {
  w: 992,
  h: 105,
  white: [
  ...
  // "NFC" a la altura y con el trazo del "AI" original
  suffix: [
    'M770 19H786V98H770Z', // N · asta izquierda
    'M818 19H834V98H818Z', // N · asta derecha
    'M770 19H787L834 98H817Z', // N · diagonal
    'M856 19H872V98H856Z', // F · asta
    'M856 19H904V34H856Z', // F · barra superior
    'M856 52H896V67H856Z', // F · barra media
  ],
  suffixStroke: 'M983.08 35.09A31.5 31.5 0 1 0 983.08 81.91', // C (trazo de 16)
};
...
function wordDefs(id: string) {
  return (
    `<linearGradient id="${id}a" x1="770" y1="19" x2="992" y2="98" gradientUnits="userSpaceOnUse">` +
...
function wordBody(id: string, mono = false) {
  ...
  const accent = mono ? 'currentColor' : `url(#${id}a)`;
  return (
    WORD.white.map((d) => `<path d="${d}" fill="${white}"/>`).join('') +
    `<path d="${WORD.s}" fill="none" stroke="${white}" stroke-width="21"/>` +
    WORD.suffix.map((d) => `<path d="${d}" fill="${accent}"/>`).join('') +
    `<path d="${WORD.suffixStroke}" fill="none" stroke="${accent}" stroke-width="16"/>`
  );
}
...
export const LOGO_VIEWBOX: Record<LogoVariant, [number, number]> = {
  mark: [MARK.w, MARK.h],
  lockup: [636 + Math.ceil(WORD.w * 1.6), MARK.h],
  stacked: [WORD.w, 523],
  word: [WORD.w, WORD.h],
};
...
export function logoSvg(variant: LogoVariant, id = 'vx', attrs = ''): string {
  ...
  if (variant === 'word') {
    ...
    defs += wordDefs(id);
    body = `<g transform="translate(${(WORD.w - MARK.w) / 2} 0)">${markBody(id)}</g><g transform="translate(0 418)">${wordBody(id)}</g>`;
  }
