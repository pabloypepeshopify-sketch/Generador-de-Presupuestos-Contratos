...
| **4** | **OpenAI → Create a completion** `openai-gpt-3:CreateCompletion` | **Estructura** el texto OCR en JSON limpio. `gpt-4o`, `temperature = 0`, modo JSON (`response_format = {type: json_object}`). *System prompt* y *user prompt* en `prompts/`. Nunca inventa: si no lee un dato, lo deja vacío. |
